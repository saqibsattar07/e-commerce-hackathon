      ...
      <div className="container mx-auto p-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {furnitureItems.map((item, index) => (
            <div key={index} className="text-center flex flex-col items-center min-h-[350px]">
              <img
                src={item.img}
                alt={item.title}
                className="mx-auto mb-4 h-40 w-auto object-contain"
              />
              <h3 className="text-base sm:text-lg md:text-xl font-medium mb-2">{item.title}</h3>
              <p className="text-lg sm:text-xl font-medium font-poppins">{item.price}</p>
            </div>
          ))}
        </div>
        <div className="text-center mt-8">
          <a href="/view-more" className="font-poppins font-medium text-lg sm:text-xl md:text-2xl text-black underline underline-offset-8">
            View More
          </a>
        </div>
      </div>
